...
| 2026-02-26 | Phase 6 | Layout Isolation: Added `usePathname` guard to Navbar & Footer (return null on portal routes). Rewrote `/login` as Student-only with Sign In/Create Account toggle. Created `/staff-login` for Recruiter/Institution/Admin. Added "Back to Website" link in dashboard sidebar. | `Navbar.tsx`, `Footer.tsx`, `login/page.tsx`, `staff-login/page.tsx`, `dashboard/student/page.tsx` | Deploy |
