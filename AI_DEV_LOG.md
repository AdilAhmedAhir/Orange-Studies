...
| 2026-02-26 | Phase 7 | Scope reduction: Deleted `/staff-login`, `/dashboard/recruiter`, `/dashboard/institution`. Created `/admin/login` (deep purple theme, Admin/Manager toggle). Updated login ghost link → `/admin/login`. Updated Navbar/Footer guards to include `/admin`, removed `/staff-login`. | `admin/login/page.tsx`, `login/page.tsx`, `Navbar.tsx`, `Footer.tsx` | Deploy |
