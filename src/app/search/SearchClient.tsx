...
function SearchContent({ courses }: { courses: CourseItem[] }) {
    ...
    const filtered = courses.filter((c) => {
        ...
        if (activeDiscipline) {
            ...
            if (d === "computer science" && !t.includes("computer") && !t.includes("software") && !t.includes("data") && !t.includes("artificial") && !t.includes("cyber") && !t.includes("robotics") && !t.includes("informatics") && !t.includes("information technology")) return false;
            if (d === "business" && !t.includes("business") && !t.includes("mba") && !t.includes("management") && !t.includes("finance") && !t.includes("marketing") && !t.includes("commerce")) return false;
            if (d === "engineering" && !t.includes("engineer") && !t.includes("mechanical") && !t.includes("civil") && !t.includes("electrical") && !t.includes("robotics") && !t.includes("environmental")) return false;
            if (d === "medicine" && !t.includes("medic") && !t.includes("health") && !t.includes("nurs") && !t.includes("pharma") && !t.includes("biomedical")) return false;
            if (d === "arts" && !t.includes("art") && !t.includes("design") && !t.includes("media") && !t.includes("creative") && !t.includes("graphic") && !t.includes("international relations")) return false;
            ...
            <div className="bg-gradient-to-r from-brand-purple via-brand-deep to-brand-purple pt-28 lg:pt-32 pb-12 px-6">
                <div className="mx-auto max-w-7xl">
                    ...
                    <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                        ...
                        {courses.length} programs across {filterGroups[0].options.length} countries
                    </motion.p>
...
export default function SearchClient({ courses }: { courses: CourseItem[] }) {
    return (
        <Suspense fallback={
