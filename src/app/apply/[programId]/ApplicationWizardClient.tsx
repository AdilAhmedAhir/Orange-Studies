...
export default function ApplicationWizardClient({
    programId,
    course,
    university,
}: {
                        ...
                        <motion.div key="step4" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.35, ease: "easeInOut" }}>
                            <div className="relative overflow-hidden rounded-2xl border border-neutral-200/60 bg-white p-12 text-center shadow-sm">
                                ...
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
                                    ...
                                    <p className="mt-1">📋 Application Reference: <strong className="text-brand-purple">{refCode || `OS-${Date.now().toString(36).toUpperCase()}`}</strong></p>
                                </motion.div>
                ...
                {currentStep < 4 && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-8 flex items-center justify-between">
                        ...
                        <button onClick={goNext} disabled={submitting}
                            className="group flex items-center gap-2 rounded-full bg-brand-orange px-8 py-3 text-sm font-bold text-white shadow-lg shadow-brand-orange/20 transition-all hover:shadow-xl hover:scale-105 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed">
                            {currentStep === 3 ? (
                                submitting ? (
