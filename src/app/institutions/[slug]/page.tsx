...
import { getUniversityBySlug } from "@/lib/mock-data";
        ...
        return (
            <div className="flex min-h-screen items-center justify-center bg-neutral-50">
                <div className="text-center">
                    <h1 className="text-3xl font-bold text-neutral-800 font-[family-name:var(--font-heading)]">University Not Found</h1>
                    <p className="mt-3 text-neutral-500">The institution you&apos;re looking for doesn&apos;t exist in our database.</p>
                    <Link href="/institutions" className="mt-6 inline-flex items-center gap-2 rounded-full bg-brand-orange px-6 py-3 font-semibold text-white transition-transform hover:scale-105">
                        ...
                        <div className="space-y-4">
                            ...
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex flex-wrap gap-3">
                                <span className="flex items-center gap-1.5 text-sm text-white/80">
                                    <MapPin className="h-4 w-4" /> {uni.location}, {uni.country.flag} {uni.country.name}
                                </span>
                                <span className="flex items-center gap-1.5 text-sm text-white/80">
                                    <Award className="h-4 w-4" /> World Rank #{uni.ranking}
                                </span>
                                <span className="flex items-center gap-1.5 text-sm text-white/80">
                                    <Clock className="h-4 w-4" /> Est. {uni.established}
                                </span>
                            </motion.div>

                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="flex flex-wrap gap-2">
                                {uni.tags.map((tag) => (
                                    <span key={tag} className="rounded-full bg-white/10 px-3 py-1 text-xs font-medium text-white border border-white/10">
                                        {tag}
                                    </span>
                                ))}
                            </motion.div>
                                ...
                                <ul className="space-y-3">
                                    {uni.highlights.map((h) => (
                                        <li key={h} className="flex items-start gap-3 text-sm text-neutral-600">
                                            <CheckCircle className="mt-0.5 h-4 w-4 shrink-0 text-brand-orange" /> {h}
                                        </li>
                                    ))}
                                </ul>
                            </div>
                            ...
                            <ul className="mt-6 space-y-3">
                                {uni.admissionRequirements.map((r) => (
                                    <li key={r} className="flex items-start gap-3 text-sm text-neutral-600">
                                        <GraduationCap className="mt-0.5 h-4 w-4 shrink-0 text-brand-purple" /> {r}
                                    </li>
                                ))}
                            </ul>
                        </div>
                        ...
                        <div className="grid gap-8 lg:grid-cols-2">
                            <div className="rounded-2xl border border-neutral-200/60 p-8">
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <h3 className="text-xl font-bold text-neutral-900 font-[family-name:var(--font-heading)]">Accommodation</h3>
                                </div>
                            ...
                            <div className="rounded-2xl border border-neutral-200/60 p-8">
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <h3 className="text-xl font-bold text-neutral-900 font-[family-name:var(--font-heading)]">Tuition Range</h3>
                                </div>
